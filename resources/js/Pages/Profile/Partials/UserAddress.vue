    ...
    <section>
        ...
        <form @submit.prevent="form.post(route('profile.address'))" class="mt-6 space-y-6">
            ...
            <div>
                ...
                <TextInput id="street" type="text" class="mt-1 block w-full" v-model="form.street" required autofocus
                    autocomplete="street" />
                <InputError class="mt-2" :message="form.errors.street" />
            </div>
            <div>
                <InputLabel for="building" value="Building" />
                <TextInput id="building" type="text" class="mt-1 block w-full" v-model="form.building" required autofocus
                    autocomplete="billing" />
                <InputError class="mt-2" :message="form.errors.building" />
            </div>
            <div>
                <InputLabel for="zipCode" value="Zip code" />
                <TextInput id="zipCode" type="text" class="mt-1 block w-full" v-model="form.zipCode" required autofocus
            ...
            <div class="flex justify-between">
                ...
                <SecondaryButton v-show="props.closeBtn" @click.prevent="closeBtn">close</SecondaryButton>
            </div>
        </form>
    </section>
</template>
...
const address = usePage().props.auth.address;
